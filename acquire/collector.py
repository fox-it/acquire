...
class Collector:
    ...
    def _output_path(self, path: str | fsutil.TargetPath, base: Optional[str] = None) -> str:
        ...
        # happend and is not needed, so this will be a no-op. However when constructing an output
    ...
    def collect_path(
        ...
        log.info("- Collecting path %s", path)
        ...
        try:
            ...
            # collect_inpath.get().  This also checks if the path can be fully resolved.
            ...
            # If the collect_inpath and branches resulting from path are all skiped due to deduping,
            ...
            log.info("- Collecting path %s succeeded", path)
